  ...
  useEffect(() => {
    ...
    const fetchCapsule = async (year: number) => {
      ...
      try {
        ...
        if (!cancelled) console.error("Failed to fetch capsule:", err);
      } finally {
    ...
    // Stagger prefetches to avoid bursting past DRF anon throttle; swallow errors (e.g. 429).
    const schedulePrefetch = (yr: number, delayMs: number) =>
      window.setTimeout(() => {
        if (cancelled || yr < 1888 || yr > 2024) return;
        moviesAPI
          .getTimeCapsule(yr)
          .then((data) => {
            if (!cancelled) {
              setCache((p) => (p[yr] ? p : { ...p, [yr]: data }));
            }
          })
          .catch(() => {});
      }, delayMs);

    const t1 = schedulePrefetch(selectedYear - 1, 400);
    const t2 = schedulePrefetch(selectedYear + 1, 900);
