...
type MovieSelectorProps = {
  ...
  movie: any;
  clear: () => void;
  onSearchQuery: (query: string, side: "A" | "B") => void;
  onSelectMovie: (tmdbId: number, side: "A" | "B") => void;
};
          ...
          {results.length > 0 && (
            <div className="absolute top-14 left-0 right-0 glass-card rounded-xl p-1.5 z-20 shadow-xl animate-fade-in">
              {results.map((m: any) => (
                <button
                  key={m.id || m.tmdb_id}
                  onClick={() => onSelectMovie(m.tmdb_id || m.id, side)}
...
export default function ComparePage() {
  ...
  const runSearch = useCallback(async (query: string, side: "A" | "B") => {
    ...
    try {
      ...
      if (side === "A") setResultsA(slice);
      else setResultsB(slice);
    } catch {
      /* ignore */
    } finally {
  ...
  const onSearchQuery = useCallback(
    (query: string, side: "A" | "B") => {
      const ref = side === "A" ? debounceA : debounceB;
  ...
  const selectMovie = useCallback(async (tmdbId: number, side: "A" | "B") => {
    setLoading(true);
    try {
